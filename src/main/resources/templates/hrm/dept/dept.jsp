...
$(function(){
	...
	$("#addDept").click(function(){
		...
		$("#divDialog").dialog({
			...
			onClose : function(){ // 关闭窗口
				window.location = "${ctx}/hrm/dept/selectDept.jspx?pageIndex=${pageModel.pageIndex}";
			}
		});
	...
	$("#deleteDept").on("click",function(){
		...
		if(checkedBoxs.length > 0){
			/** 提示确认删除吗?*/
			$.messager.confirm('模块管理', '确认删除部门吗?', function(r){
				if (r){
				    /** 获取选中的数据行模块id */
				    ...
				    /** 获取需要删除的模块的id */
				    window.location = "${ctx}/hrm/dept/deleteDept.jspx?pageIndex=${pageModel.pageIndex}&ids="+idMaps.get();
				}
			});		
		}else{
			 $.messager.alert('错误提示',"请选择需要删除的部门!",'error');
		}
	})
    ...
    function updateDept(id){
    	...
    	/** 获取当前选中的数据行数 */
			...
			/** div弹出以后，立即加载界面，并且显示界面  */
		     $("#iframe").attr("src", "${ctx}/hrm/dept/showUpdateDept.jspx?id="+id).show();
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				...
				<fkjava:pager pageIndex="${pageModel.pageIndex}" 
				  ...
				  submitUrl="${ctx}/hrm/dept/selectDept.jspx?pageIndex={0}"/>
			</div>
